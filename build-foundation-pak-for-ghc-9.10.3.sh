...
download_ghc_source_code() {
  wget https://downloads.haskell.org/~ghc/9.10.3/ghc-9.10.3-src.tar.xz
}

unpack_ghc_source_code_and_add_hadrian_to_git() {(
  tar xf ghc-9.10.3-src.tar.xz

  cd ghc-9.10.3/hadrian
  git init
  set +e
  git add *
...
build_stage2_ghc() {(
  # does not compile with ghc 9.10.3
...
build_wpc_plugin_with_stage2_ghc() {(
  STAGE2_GHC=`pwd`/ghc-9.10.3/_build/stage1/bin/ghc
  ...
  # wpc-plugin for GHC 9.10.3
  git checkout 6c0bced14e805ef8ccdbf6edc153068ab5b55687
...
mkdir -p foundation-pak-ghc-9.10.3-wpc
cd foundation-pak-ghc-9.10.3-wpc
...
############
# output foundation pak
############

ls -lah `pwd`/ghc-9.10.3/_build/foundation-pak/*.tar.*
